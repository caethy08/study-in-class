...
if(jwt != null){
	try{
		...
		Jws<Claims> claims = Jwts.parserBuilder().setSigningKey(secretKey) //서명 키 설정 JWT생성 시 사용된 secret key와 일치해야함
								.build() //파서 객체 생성
								.parseClaimsJws(jwt);
		String userid = claims.getBody()    //jwt의 playload를 반환
							.getSubject(); //subject의 클레임 반환
		//유효한 경우 환영 메세지 출력
%>
