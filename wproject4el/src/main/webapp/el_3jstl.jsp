...
<hr>
** 변수 처리 ** <br>
<c:set var="irum" value="이기자" scope="page"></c:set> <%-- page, requeset, session, application 사용 가능--%>
이름 : <c:out value="${irum}"></c:out>
	<%--  <c:out value="${irum}"/>  위와 동일--%>
<br>
...
abc값은 (현재 사용 중인 브라우저 정보) <c:out value="${abc}" />
<br>
...
<c:if test="${nice == 'star'}"> <%-- ${nice eq 'star'}도 앞과 동일 --%>
	if 연습 : nice값은 <c:out value="${nice}"></c:out>
</c:if>
<p/>
** 조건 판단문 choose ** <br/>
<c:choose>
...
<c:choose>
	...
	<c:otherwise>
		회원 <c:out value="${param.myid}"/>님 어서오세요
	</c:otherwise>
</c:choose>
<hr>
** 반복문 foreach ** <br>
...
<%-- 숫자 표기에서 #은 유효인 숫자만 처리하고 무효면 공백, 0은 무효의 숫자를 0으로 처리한다. --%>
<br>
