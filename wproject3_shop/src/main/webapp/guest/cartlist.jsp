...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<jsp:useBean id="cartMgr" class="pack.order.CartMgr" scope="session" /> <%-- 세션이 살아있는 한 유효 --%>
...
<table>
	<tr style="background-color: orange;">
		<th>주문 상품</th><th>가격(소계)</th><th>주문 수량</th><th>수정/삭제</th><th>조회</th>		
	</tr>
<% 
int totalPrice = 0;
...
<%
	...
	*/
	...
	for(Map.Entry<String, OrderBean> entry : hCart.entrySet()){//위는 while로 반복하나 여기는 for로 빈복 처리
		OrderBean orderbean = entry.getValue();  //위의 예쩐 방법과 가져오는 방법이 다르디
		ProductDto product = productMgr.getProduct(orderbean.getProduct_no());
		int price = Integer.parseInt(product.getPrice());
		int quantity = Integer.parseInt(orderbean.getQuantity());
		int subTotal = price * quantity; //소계
