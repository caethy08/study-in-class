...
public class ProductMgr {
	...
	public boolean insertProduct(HttpServletRequest request) {
		...
		try {
			...
			String uploadDir = "C:/Users/user/git/study-in-class/wproject3_shop/src/main/webapp/upload";
			...
			conn = ds.getConnection();
			String sql = "insert into shop_product(name, price, detail, sdate, stock, image) values(?,?,?,now(),?,?)"; //no는 자동증가여서 제외
			pstmt = conn.prepareStatement(sql);
	...
	*/
	...
	public boolean deletProduct(String no) {
		boolean b = false;
		...
		try {
			...
			System.out.println("deletProduct err : " + e);
		} finally {
			try {
				if(rs != null) rs.close();
			if(pstmt != null) pstmt.close();
