...
public class MemberMgr {
	...
	public boolean memberUpdate(MemberBean memberBean, String id) {
		...
		try {
			...
			pstmt.setString(8, id);  //그냥 받아오는 아이디여서 get아님
			if(pstmt.executeUpdate() > 0) b = true;
		} catch (Exception e) {
			System.out.println("btnUpdate err : " + e);
		} finally {
			try {
				if(rs != null) rs.close();
			if(pstmt != null) pstmt.close();
	...
	//관리자가 전체 회원자료 읽기
	public ArrayList<MemberBean> getMemberAll() { //원래는 Dto사용을 더 권장하나 안만들어서 대용인 bean을 사용
