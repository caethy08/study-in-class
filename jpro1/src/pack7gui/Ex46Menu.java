...
public class Ex46Menu extends JFrame implements ActionListener{
	...
	private void addLayout() {
		...
		add("South", panel); //frame은 boarderlayout이므로
		add("Center", txtArea);
	...
	private void addmenuLAyout() {
		...
		setJMenuBar(mbar); //메뉴바를 Jframe에 등록
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btnR) {
			...
			//내장 다이얼로그 박스 호출
			JOptionPane.showMessageDialog(this, "메세지", "알림", JOptionPane.INFORMATION_MESSAGE);// 현재 프레임에서 메세지 다이알로그띄움
			//modal dialogbox는 실행을 멈추고 창을 보여준다. 창을 닫으면 처리 진행
			//유저가 메세지 창을 다 볼때까지(창이 닫힐때까지) 작업 일시 중지
