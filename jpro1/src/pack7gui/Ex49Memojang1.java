...
// 간단한 메모장 만들기
public class Ex49Memojang1  extends JFrame implements ActionListener {

	private JTextArea txtMemo = new JTextArea("",10,30);
	private String copyText; //복사한 무나열 잠시 저장
	...
	public Ex49Memojang1 () {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) { // 무명 클래스여서 this를 지정해줘야한다.
				int close = JOptionPane.showConfirmDialog(Ex49Memojang1 .this, "종료하시겠습니까?", "종료", JOptionPane.YES_NO_OPTION); //내부 무명클래스에서는 클래스명을 넣어줘야한다.
	...
	private void menuLayout() {
		...
		txtMemo.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				getModifiersEx() : 마우스 또는 키보드 이벤트가 발생할 때 해당 이벤트와 함께 어떤 추가 키(ex. Shift, Control)가 눌렸는지 확인하는데 사용
				*/
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == mnuNew) {// 새문서
			...
			try {
				...
				writer.close();
				setTitle("c:/work/a.txt");
			} catch (Exception e2) {
				System.out.println(e2.getMessage()); //개발자가 보기위함
				JOptionPane.showMessageDialog(this, e2.getMessage(), "에러", JOptionPane.WARNING_MESSAGE); //사용자에게 보여줌
			}
		}else if (e.getSource() == mnuExit) { // 종료
			int result = JOptionPane.showConfirmDialog(this, "정말 종료할까요?", "종료", JOptionPane.YES_NO_OPTION);
			...
			//System.out.println(txtMemo.getSelectedText()); //범위 지정한 부분만
			copyText = txtMemo.getSelectedText();
			
		}else if (e.getSource() == mnuPaste) { // 붙여넣기
			//txtMemo.setText(copyText); 저장한거 날아감
			...
			System.out.println("About 호출 후 상황");// modal, modaless 구분용
			...
			try {
				...
				if (Desktop.isDesktopSupported()) {
					...
					if (desktop.isSupported(Desktop.Action.BROWSE )) { //클래스의 포함관계
						//uri는 문자열, 특정 리소스 의미
		...
		}else if (e.getSource() == m_white) {// 팝업 메뉴판
			txtMemo.setBackground(Color.WHITE);
		}else if (e.getSource() == m_blue) {// 팝업 메뉴판
			txtMemo.setBackground(Color.BLUE);
		}else if (e.getSource() == m_yellow) {// 팝업 메뉴판
