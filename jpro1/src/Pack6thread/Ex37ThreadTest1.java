...
public class Ex37ThreadTest1 extends Thread {//tread는 추상 클래스를 상속
	...
	public static void main(String[] args) {
		// Thread
		// 하나의 응용 프로그램은 운영체제에 의해 process(작업단위)를 확봏하고 
		//응용 프로그램의 실행은 thread(실행 단위)가 담당한다.
		//기본적으로 main thread에 의해 응용프로그램이 실행된다.
		//tread의 갯수만큼 실행 단위를 늘릴 수 있다. 
		...
		try {
			...
			//thread를 사용하지 않은 경우: 순차적으로 실행/동시진행 불가능(메인 스레드 하나에서 작업)
			/*
			...
			*/
			...
			th2.setPriority(MAX_PRIORITY); //스레드 스케쥴러에게 우선 순위 요청. 확률이 높아지는 것(확정이 아님)
			
			th1.join(); //th1의 스레드가 종료될때까지 main 스레드 종료를 대기("프로그램 종료"를 안만난다)
			th2.join();
